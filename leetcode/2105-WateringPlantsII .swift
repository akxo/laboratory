...
Alice waters the plants in order from left to right,
starting from the 0th plant. Bob waters the plants in order from right to left,
starting from the (n - 1)th plant. They begin watering the plants simultaneously.

It takes the same amount of time to water each plant
regardless of how much water it needs.
...
and two integers capacityA and capacityB
...
- Alice has enough water for plant 1, so she waters it.
Bob does not have enough water for plant 2, so he refills his can then waters it.
So, the total number of times they have to refill
to water all the plants is 0 + 0 + 1 + 0 = 1.
...
Output: 2
Explanation:
- Initially, Alice and Bob have 3 units and 4 units of water
in their watering cans respectively.
- Alice waters plant 0, Bob waters plant 3.
- Alice and Bob now have 1 unit of water each,
and need to water plants 1 and 2 respectively.
- Since neither of them have enough water for their current plants,
they refill their cans and then water the plants.
So, the total number of times they have to refill
to water all the plants is 0 + 1 + 1 + 0 = 2.
...
class Solution {
    func minimumRefill(_ plants: [Int], _ capacityA: Int, _ capacityB: Int) -> Int {
        ...
        var canB = capacityB
        var alice = 0
        var bob = plants.count - 1

        while alice <= bob {
            if alice == bob {
                if max(canA, canB) < plants[alice] { refills += 1 }
                break
            } else {
                if canA < plants[alice] {
                    canA = capacityA
                    refills += 1
                }
                canA -= plants[alice]
