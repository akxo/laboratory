/* You are working in a ball factory where you have n balls
numbered from lowLimit up to highLimit inclusive (i.e., n == highLimit - lowLimit + 1),
...
class Solution {
    func countBalls(_ lowLimit: Int, _ highLimit: Int) -> Int {
        ...
        for ball in lowLimit...highLimit {
            let boxNum = String(ball).reduce(0) { $0 + Int(String($1))! }
            boxCount[boxNum, default: 0] += 1
        }
        
        return boxCount.values.max() ?? 0
    }
}
