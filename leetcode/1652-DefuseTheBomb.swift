     ...
     func decrypt(_ code: [Int], _ k: Int) -> [Int] {
         if k == 0 {
             ...
             for index in code.indices {
                 var sum = 0
                 for increment in 1...k {
                     let nextIndex = (index + increment) % code.count
                     sum += code[nextIndex]
                 }
                 newCode[index] = sum
             }
             ...
             for index in code.indices {
                 var sum = 0
                 for decrement in 1...abs(k) {
                     var prevIndex = (index - decrement) % code.count
                     prevIndex = prevIndex < 0 ? prevIndex + code.count : prevIndex
                     sum += code[prevIndex]
                 }
                 newCode[index] = sum
             }
